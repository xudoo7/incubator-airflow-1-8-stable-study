...
class DagFileProcessor(AbstractDagFileProcessor):
        ...
        def helper():
            ...
            try:
                ...
                logging.info("Started process (PID=%s) to work on %s",
                             os.getpid(),
                             file_path)
                # 调用SchedulerJob的Process_file处理dag文件
    ...
    def start(self):
        ...
        """
        # 调用_launch_process创建处理file_path进程
...
class SchedulerJob(BaseJob):
    ...
    def create_dag_run(self, dag, session=None):
        ...
        if dag.schedule_interval:
            ...
            if len(active_runs) >= dag.max_active_runs and not dag.dagrun_timeout:
                return
            timedout_runs = 0
            # 当前时间减掉dr超时时间大于dr开始时间，则dr执行失败，更新dr状态及结束时间
            ...
            # DagRun对query操作进行了封装，以实现了find的方法，此处可以用find方法
            qry = (
            ...
            if next_run_date and min_task_end_date and next_run_date > min_task_end_date:
                return
            # 上面的逻辑balabala，至此把dag写入到dagrun表
    ...
    def _process_task_instances(self, dag, queue):
        ...
        for run in dag_runs:
            ...
            # 核实dagrun，更新dagrun已删除任务状态，还没有在TI表中任务为其创建TI。
    ...
    def _process_dags(self, dagbag, dags, tis_out):
        ...
        for dag in dags:
            ...
            # 如果是dag实例且dag实例is_paused为false，则创建dag_run
    ...
    def _execute(self):
        ...
        # user code.
        # 通过多进程并行从DAGS中获取、生成任务。由于是在各自的进程中处理DAG，所以能够并行并且隔离
        #用户有害代码
        ...
        #定义DAG工厂方法，返回DagFileProcessor实例
    ...
    def _execute_helper(self, processor_manager):
        ...
        # 查找DagRun表，筛选所有状态为running的DagRun实例
        active_runs = DagRun.find(
            state=State.RUNNING,
            external_trigger=False,
            session=session,
        ...
        #遍历active_runs，重置TI状态为“调度”或“排除”，但执行器中没有执行的TI状态为None
        for dr in active_runs:
        ...
        while (datetime.now() - execute_start_time).total_seconds() < \
            ...
            #如果刷新耗时大于DGA目录遍历用时，则重新获取dag文件路径
            if elapsed_time_since_refresh > self.dag_dir_list_interval:
                ...
                self.logger.info("There are {} files in {}"
                                 .format(len(known_file_paths), self.subdir))
                #根据know_file_paths更新文件队列及_process映射
            ...
            #Call DagFileManager的hearbeat，多进程处理DAG定义文件，写入DR\TI,把任务放入队列，返回simpledag
