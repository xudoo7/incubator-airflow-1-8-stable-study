...
class DagBag(BaseDagBag, LoggingMixin):
    ...
    def process_file(self, filepath, only_if_updated=True, safe_mode=True):
        ...
        try:
            ...
            # race condition
            # 如果filepath在self.file_last_changed中，且文件没有变化，则返回found_dags
        ...
        if not zipfile.is_zipfile(filepath):
            if safe_mode and os.path.isfile(filepath):
                with open(filepath, 'rb') as f:
                    content = f.read()
                    # 如果文件不是DAG文件，那写入file_last_changed字典，并返回found_dags
        ...
        for m in mods:
            ...
            for dag in list(m.__dict__.values()):
                ...
                if isinstance(dag, DAG):
                    ...
                    dag.is_subdag = False
                    #生成self.dags，把dag实例及其dag_id写入self.dags字典
                ...
                for f in files:
                    try:
                        ...
                        if file_ext != '.py' and not zipfile.is_zipfile(filepath):
                            continue
                        # 如果文件非忽略文件，则调用self.process_file处理
                        if not any(
...
class DAG(BaseDag, LoggingMixin):
    ...
    def subdags(self):
        ...
        l = []
        # 遍历dag的tasks，如果有task是SubDagOperator类，且有subdag属性，则写入列表，最后返回
